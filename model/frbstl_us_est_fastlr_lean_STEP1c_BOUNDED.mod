...
estimated_params;
  ...
  rho_gz,   beta_pdf, 0.500, 0.200;
  // rho_mup sits at 0.9989 -> the finite-difference Hessian steps past 1 and returns
  // NaN for this whole row/column. Explicit upper bound keeps the step stationary.
  rho_mup,  0.9989, 0.0010, 0.9995, beta_pdf, 0.500, 0.200;
  ...
  // rho_tr sits at 0.9977 -> same problem, same fix.
  rho_tr,   0.9977, 0.0010, 0.9995, beta_pdf, 0.500, 0.200;
...
// STEP 1c: re-run the mode WITH rho_mup and rho_tr bounded below 1 (see the
// estimated_params block above).  Start from the mode you already found.
// Afterwards the Hessian should be complete: no NaN rows, positive definite.
estimation(datafile = 'usdata_1959_2026.mat',
           mode_compute = 5, mode_file = frbstl_us_est_fastlr_mode_ndc,
           mh_replic = 0,
           presample = 4, lik_init = 2, nograph);
