...
// STEP 2 of 3: PILOT.  mode_compute = 0 -> NO mode search: the mode and the
// repaired Hessian are read from the file.  Do NOT re-run step 1/1c after the
// repair, it would overwrite the fixed file with a fresh NaN Hessian.
// PILOT, 5k draws, 8 chains on 8 physical cores.
//   dynare frbstl_us_est_fastlr_lean parallel conffile=cluster.ini
// If the Hessian is STILL not positive definite after step 1b, swap
//   MCMC_jumping_covariance = hessian   ->   = prior_variance
// It is a cruder proposal, but it is positive definite and mh_tune_jscale adapts.
estimation(datafile = 'usdata_1959_2026.mat',
