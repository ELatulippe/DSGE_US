...
// STEP 1b: SECOND PASS with csminwel, starting FROM the newrat mode.
// newrat returns the outer-product-gradient Hessian, which came back NOT positive
// definite (min eig 1.8e-4, max 2.6e9, condition ~1e13).  MCMC_jumping_covariance =
// hessian cannot use that.  csminwel builds its Hessian differently and is usually
// better behaved.  Point mode_file at whatever STEP 1 wrote.
estimation(datafile = 'usdata_1959_2026.mat',
           mode_compute = 4, mode_file = frbstl_us_est_fastlr_mode_ndc,
           mh_replic = 0,
           presample = 4, lik_init = 2, nograph);
