...
//
//  ------------------------------------------------------------------ HOW TO RUN
//  Prereq: you already ran STEP1/STEP2/STEP3 to convergence, which wrote the
//  repaired mode file. On Dynare 7.1 it lands in ./<base>/Output/ .
//    1) Copy the mode file next to THIS .mod (or edit the mode_file path below):
//         cp <base>/Output/frbstl_us_est_fastlr_mode_ndc_fixed.mat .
...
//  this file still runs with mode_compute=0 on frbstl_us_est_fastlr_mode_ndc_fixed.mat.
...
// natural economy = flex prices/wages with inflation at the TIME-VARYING target,
// Pi^f = Pi_star (paper Sec. 2.12, item 3), so the nominal natural rate is
// rstar*Pi_star(+1).  Pi_star has SS Pibar, so the flex SS still coincides with
// the sticky one.  NOTE: debt is nominal and workers are non-Ricardian, so this
// is NOT a normalization: Pi^f moves real debt through b(-1)/(Pi^f*Gamma).
(1+tau_c)*(cwN_f+pO_f*cwO_f) + bw_f/(rstar*Pi_star(+1)) = (1-tauL_f)*w_f*n_f/lam + bw_f(-1)/(Pi_star*Gamma) + tr_eff; // (46f)
...
(1-tauK_f)*R_k_f = Apnu_f;                                                           // (51f) corrected
Q_f = SDF_s_f*((1-tauK_f(+1))*R_k_f(+1)*nu_f(+1)-Anu1_f+(1-delta)*Q_f(+1));           // (52f)
...
g + tr_eff + bg_f(-1)/(Pi_star*Gamma) = tau_c*(cN_f+pO_f*cO_f)+tauL_f*w_f*n_f+tauK_f*(R_k_f*ku_f+d_f)+bg_f/(rstar*Pi_star(+1)); // (76f)
...
// ============================================================================
//  FAST STANDALONE FEVD + IRF RUNNER  (no smoother/MH -> runs in ~1-2 min)
//  Loads the posterior mode, solves the model, and produces:
//    * oo_.conditional_variance_decomposition  -> Forecast Error Variance Decomp
//    * oo_.irfs (incl. *_e_mp, rstar/ogap)      -> monetary-policy counterfactual
//  Run:   dynare fevd_run
//  Then:  make_fevd            make_counterfactual
//  (Needs frbstl_us_est_fastlr_mode.mat + usdata_1959_2026.mat in this folder.)
// ============================================================================
estimation(datafile = 'usdata_1959_2026.mat',
           mode_file = frbstl_us_est_fastlr_mode_ndc_fixed,
           mode_compute = 0, mh_replic = 0,
           presample = 4, lik_init = 2, nograph, nodiagnostic, cova_compute = 0);

stoch_simul(order = 1, irf = 40, nograph, noprint,
            conditional_variance_decomposition = [1 4 8 16 40])
  gdp gdp_f cc iv n Pi Pi_h R w d pO rstar ogap obs_gdp obs_pi obs_ffr;
// irf=40 -> matches the paper's Appendix-A IRF horizon (make_irf_panel.m reads oo_.irfs)
