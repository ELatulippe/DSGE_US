...
// STEP 1 of 3: RE-FIND THE MODE (newrat).
//
// mode_check REMOVED on purpose: it perturbs each parameter over a grid, and some
// grid points push the steady state into a region where it goes complex, which
// kills the run AFTER the mode is found.
// Keep mode_file -- from the prior means the optimizer wanders and static_resid errors.
//
// MaxIter = 7.  newrat alternates Sims' csminit1 line search along the Newton
// direction with bounded univariate steps (mr_gstep).  The estimated_params BOUNDS
// constrain only the univariate phase and the prior support -- csminit1 is
// unconstrained, so a long Newton step walks alpha negative (or nu_C / nu_Y through
// 0), the steady state goes complex and the static_resid MEX throws
// "y must be a real dense numeric array".  That is what killed iteration 8 twice,
// at exactly the same point both times (|dx| = 45.321): the trajectory is
// deterministic, so capping the iteration count is a reliable way to stop short of
// it and let Dynare finish, compute the Hessian and write the mode file normally.
//
// Stopping at 7 costs essentially nothing.  The per-iteration improvements were
//   0.104  0.118  0.123  0.094  0.043  0.026  0.013
// i.e. halving, so the whole remaining gain is about 0.013 in log posterior across
// 63 parameters.  Ftol = 1e-5 is chasing numerical noise at that point, and the
// mode only has to be good enough to start the chains and shape the proposal.
//
// If Dynare rejects the optim syntax, the equivalent is to loosen the tolerance:
//   optim = ('TolFun', 0.02)      // stops after iteration 6 on this trajectory
// and if neither is accepted, recover the iteration-7 iterate from m1.mat with
// scripts/mode_from_m1.m instead.
estimation(datafile = 'usdata_1959_2026.mat',
           mode_compute = 5, mode_file = frbstl_us_est_fastlr_mode_ded_fixed,   // start from the previous branch's mode: closest point we have
           optim = ('MaxIter', 7),
           mh_replic = 0,
           presample = 4, lik_init = 2, nograph);
