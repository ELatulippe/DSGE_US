...
// the existing 8x250k draws WITHOUT resampling (mh_replic=0, load_mh_file).
//
...
estimation(datafile = 'usdata_1959_2026.mat',
  ...
  Pi_star obs_inflexp me_inflexp                   // inflation target, 10y infl-exp, and its measurement
                                                   // error: the three objects diag_inflation_target.m reads.
                                                   // (109) is obs_inflexp = 100*(exp(4*piq10)-1+me_inflexp),
                                                   // so 100*me_inflexp is the wedge in pp between the model's
                                                   // own 10y expected headline inflation and the survey.
