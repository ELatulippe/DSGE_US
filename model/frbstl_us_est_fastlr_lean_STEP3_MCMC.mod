...
// STEP 3 of 3: FULL MCMC.  mode_compute = 0 -> reads the repaired mode file.  8 chains on 8 physical cores = ONE wave.
// 8 x 250k, mh_drop=0.5 -> 1M post-burn-in draws, matching the paper's total.
//   dynare frbstl_us_est_fastlr_lean parallel conffile=cluster.ini
estimation(datafile = 'usdata_1959_2026.mat',
           mode_compute = 0, mode_file = frbstl_us_est_fastlr_mode_ndc_fixed,   // repaired Hessian, see scripts/repair_mode_hessian.m
           mh_replic = 250000, mh_nblocks = 8, mh_drop = 0.5,
           mh_jscale = 0.23046,   // TUNED BY THE STEP-2 PILOT ON THIS MODEL (not the superseded
                                  // deductible one, whose 0.23504 is discarded). The pilot hit
                                  // 31.4-33.9% acceptance across all 8 blocks at this value, so
                                  // mh_tune_jscale is dropped: its tuning pass costs a preliminary
                                  // chain and would only rediscover the same number.
           MCMC_jumping_covariance = hessian,
           presample = 4, lik_init = 2, nograph);
