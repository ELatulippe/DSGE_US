...
% ---- DROP THE PRESAMPLE -----------------------------------------------------
% The estimation uses presample = 4: the first four quarters initialise the
% Kalman filter and never enter the likelihood.  With lik_init = 2 the initial
% state carries a huge prior variance, so the smoother's first few values are
% essentially unidentified.  Left in, 1959Q1 alone supplied the "most negative
% gap = -6.9%" and the entire gap-vs-ogap discrepancy -- an artefact of
% initialisation being compared against the paper's 2020Q2 trough.
NPRE = 4;
g=g(NPRE+1:end); gf=gf(NPRE+1:end); yr=yr(NPRE+1:end); T=numel(g);
if isfield(SV,'ogap'); SV.ogap = SV.ogap(NPRE+1:end); end
fprintf('\n[dropped the first %d quarters: presample, not identified by the likelihood]\n', NPRE);
...
% The note centres the gap on its LONG-RUN FORECAST level, not on the sample
% mean, and make_paper_figures does the same. Those differ by about 2 pp here
% (+3.3 vs +5.2), so using the sample mean in this script made the two disagree
% on every historical reading. Use the long-run level when the forecast is in
% oo_, and say so; fall back to the sample mean only if it is absent.
off = NaN; offsrc = '';
FC = struct();
...
if isfield(FC,'gdp') && isfield(FC,'gdp_f')
  fg=FC.gdp(:); fgf=FC.gdp_f(:); n=min(numel(fg),numel(fgf));
  if n>=8
    tail = max(1,n-19):n;                      % last ~5 years of the forecast
    off  = mean(100*(log(fg(tail))-log(fgf(tail))));
    offsrc = 'long-run forecast level (the note''s convention)';
  end
end
if ~isfinite(off); off = mean(gap); offsrc = 'sample mean (FALLBACK: no forecast in oo_)'; end
gapc = gap - off;
fprintf('---- CENTERED gap (offset %+.2f pp removed; %s) ----\n', off, offsrc);
fprintf('     sample mean for comparison: %+.2f pp\n', mean(gap));
...
% NB: exist(...,'dir') searches the MATLAB PATH, so a 'figures' folder anywhere on
% the path makes it return true and the mkdir is skipped, while the relative print
% path still resolves to the CURRENT folder -> "Cannot create output file".
% isfolder() does not search the path.  Write alongside the other scripts' output.
OUT='figures_paper';
if exist('isfolder','builtin') || exist('isfolder','file')
  if ~isfolder(OUT); mkdir(OUT); end
else
  if exist(fullfile(pwd,OUT),'dir')~=7; mkdir(OUT); end
end
fn=fullfile(OUT,'diag_gdp_vs_gdpf.png');
