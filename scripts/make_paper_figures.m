...
%     Fig 6  Model r* (1y & 5y forward) vs external LW / Lubik-Matthes (slots)
...
% data (FRB NY = Laubach-Williams (one-sided); FRB Richmond = Lubik-Matthes): drop
% them into LW / LM below, quarterly-aligned to yrr, to complete the figure.
try
  ...
  % ---- external comparators: read rstar_external.csv (date,LM_median,LW_rstar) ----
  %   LM = Lubik-Matthes (Richmond Fed) -- shipped filled 1985Q1-2026Q1.
  %   LW = Laubach-Williams, one-sided (NY Fed) -- blank until you run
  %         add_lw_to_external.m on the official xlsx (see that file's header).
  ...
  if ~isempty(ext.LW) && any(isfinite(ext.LW))
    plot(yrr, ext.LW, '--','color',[0.4 0.4 0.4],'linewidth',1.5);
    leg{end+1}='Laubach-Williams (one-sided)'; yref=[yref ext.LW(isfinite(ext.LW))'];
  end
...
% read rstar_external.csv (date,LM_median,LW_rstar) -> vectors aligned to yrr (NaN where absent)
function ext = read_external_rstar(yrr)
ext = struct('LM',[],'LW',[]);
fn = 'rstar_external.csv';
if ~exist(fn,'file'); return; end
LM = nan(numel(yrr),1); LW = nan(numel(yrr),1);
...
while ~feof(fid)
  ...
  if numel(p)>=3 && ~isempty(strtrim(p{3})); LW(k) = str2double(p{3}); end
end
fclose(fid);
ext.LM = LM; ext.LW = LW;
end
