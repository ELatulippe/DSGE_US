...
&=P_{t-1}B^s_{t-1}+(1-\tau^k_t)P_t\big(\nu_t R^k_t K_{t-1}+D_t\big)\nonumber\\
&\quad-P_t A(\nu_t)K_{t-1}+P_t T_t+P^O_t O_{S,t},\tag{P-5}\label{eq:cap-bc}
\end{align}
...
the adjustment cost $S(I_t/I_{t-1})$ links consecutive investment levels.
...
+\Lambda_t\Big[P_{t-1}B^s_{t-1}+(1-\tau^k_t)P_t(\nu_t R^k_t K_{t-1}+D_t)-P_tA(\nu_t)K_{t-1}\nonumber\\
...
so that $\beta\,\Lambda_{t+1}/\Lambda_t=m^s_{t,t+1}/\Pi_{t+1}$. Adding the convenience-yield wedge
$\vartheta_t$ (see \hyperref[rem:conv]{Remark~\ref*{rem:conv}}),
\begin{equation}\label{eq:9}
\boxed{\,1=R_t\,\E_t\!\Big[\frac{m^s_{t,t+1}}{\Pi_{t+1}}\Big]\vartheta_t\,}
\tag{P-9}
\end{equation}

\begin{remark}[Convenience yield]\label{rem:conv}
The value function (3) as written has no bonds in utility, which yields
$1=R_t\,\E_t[m^s_{t,t+1}/\Pi_{t+1}]$. The paper's footnote notes that $\vartheta_t$ can be
micro-founded by a bonds-in-utility term; here we insert it as a reduced-form multiplicative wedge on
the return to government debt, exactly as in Del~Negro et al.\ (2017). Because $\vartheta_t$ is known
at $t$ it can be taken outside the expectation.
\end{remark}

\subsection{Capital utilization \texorpdfstring{$\Rightarrow$}{=>} eq.\ (12)}
$\nu_t$ enters only the period-$t$ budget, through $(1-\tau^k_t)P_t\nu_t R^k_t K_{t-1}$ (taxed income)
and $-P_tA(\nu_t)K_{t-1}$ (untaxed cost). Differentiating and dividing by $\Lambda_t P_t K_{t-1}$,
\begin{equation}\label{eq:util-derived}
(1-\tau^k_t)\,R^k_t=A'(\nu_t).
\end{equation}

\begin{remark}[Discrepancy in the utilization condition]\label{rem:util}
\flag{The paper states eq.\ (12)/(51) as $R^k_t=A'(\nu_t)$}, without the tax factor, and its
functional-form calibration sets $A'(1)=\kappa_a=\bar R^k$ (p.\ 13), which is consistent with
$R^k_t=A'(\nu_t)$ but \emph{not} with \eqref{eq:util-derived}. The tension is purely about whether the
utilization cost $A(\nu_t)K_{t-1}$ is tax-deductible:
\begin{itemize}[nosep]
  \item Budget (5) and Tobin's~Q (10) both place $A(\nu_t)$ \emph{outside} the $(1-\tau^k_t)$ bracket
  (utilization cost \emph{not} deductible). Taken literally this gives \eqref{eq:util-derived}.
  \item Eq.\ (12) and the calibration $A'(1)=\bar R^k$ require $A(\nu_t)$ to sit \emph{inside} the
  bracket (cost deductible), which cancels $(1-\tau^k_t)$ from both marginal revenue and marginal cost
  and yields $R^k_t=A'(\nu_t)$.
\end{itemize}
The cleanest internally-consistent reading is: utilization income and cost are taxed symmetrically, so
the FOC is $R^k_t=A'(\nu_t)$ \emph{and} the term in (10) should read
$(1-\tau^k_{t+1})\big[\nu_{t+1}R^k_{t+1}-A(\nu_{t+1})\big]$. This matters for the Canadian
recalibration because it changes the steady-state link between $\bar R^k$, $\kappa_a$, and $\bar\tau^k$.
I flag it here rather than silently ``correcting'' the paper.
\end{remark}
...
\subsection*{Utilization, restated}
For completeness, the utilization condition derived above is (subject to
\hyperref[rem:util]{Remark~\ref*{rem:util}}) the paper's
...
\begin{remark}[Discounting convention]
The paper's recursion \eqref{eq:union} discounts the continuation by $\beta/\Pi_{t+1}$ rather than by
the worker's full nominal SDF $m^w_{t,t+1}/\Pi_{t+1}$. We follow the paper's stated form verbatim so
that the FOC \eqref{eq:union-foc} matches the text term-for-term; this is the convention embedded in
the published wage Phillips curve (26).
\end{remark}
...
\begin{remark}[Reading the wage Phillips curve]
Ignoring adjustment costs ($\eta_w=0$), \eqref{eq:26} reduces to the static labor-supply condition
$W_t/P_t=\MRS_t(1+\varphi)\,\mu^w_t/(\mu^w_t-1-\mu^w_t)$; more transparently, the flex-wage limit sets
the real wage as a markup $\mu^w_t$ over the ``competitive'' object $\MRS_t(1+\varphi)$, matching the
paper's flexible-wage condition $W_t/P_t=\MRS_t(1+\varphi)$ once the markup is normalized. The two
inflation terms are the standard forward-looking Rotemberg wedge, indexed to $\Pi^{w,\Ind}_t$.
\end{remark}
...
\begin{remark}[The firm's discount factor]\label{rem:nomsdf}
\flag{The paper's recursion (28) writes the continuation as $\E_t m^s_{t,t+1}V^f[\cdot]$}, i.e.\ with
the \emph{real} SDF, even though the flow payoffs in (28) are nominal (every term carries a price
level). Because a nominal claim must be priced with the nominal kernel $M_{t,t+1}=m^s_{t,t+1}/\Pi_{t+1}$,
the extra $1/\Pi_{t+1}$ is exactly what cancels a spurious $\Pi_{t+1}$ in the forward term and delivers
the published NKPC (36)/(62). Equivalently, one may deflate the whole problem and discount \emph{real}
profits by $m^s_{t,t+1}$. We use the nominal kernel \eqref{eq:nomsdf} so the result matches the paper.
\end{remark}
...
\tag{P-36}
\end{equation}
which is eq.\ (36). In the flexible-price limit ($\eta_p\to\infty$ costs removed, or the static case)
...
=\tau^c(P_tC_{N,t}+P^O_tC_{O,t})+\tau^l_tW_tN_t+\tau^k_tP_t(\nu_tR^k_tK_{t-1}+D_t)+P_t\frac{B^g_t}{R_t}\,}\tag{P-38}
\end{equation}
...
capital income plus profits at $\tau^k_t$.
...
consumption tax $\tau^c$ is constant. Note the Taylor rule strips out trend growth $\Gamma_t$ so it
...
\begin{remark}[The rest of the system]
...
=\frac{\bar\Gamma/\beta-(1-\delta)}{1-\bar\tau^k},\qquad \kappa_a=\bar R^k. \label{eq:ss-prices2}
\end{align}
...
Phillips curve (56) --- with the Rotemberg gaps zero --- reduces to a static labor-supply condition,
...
from (80), $\bar d=\bar y-\bar w\bar n-\bar R^k\bar k^u-\bar p^O\bar o_Y$. By Walras' law the capitalist
budget and the government budget (76) hold automatically --- a useful numerical check.

\begin{remark}[Steady-state real wage carries the wage markup]\label{rem:wagess}
\flag{The static condition \eqref{eq:ss-wage} contains the wage markup $\bar\mu^w$}, i.e.\
$\bar w=\bar\mu^w(1+\varphi)\overline{mrs}$, which is the exact $\eta_w\!\to\!0$ limit of the wage
Phillips curve (56)/(26) derived in Section~\ref{sec:unions}. The paper's flexible-wage condition on
p.\ 16 is written $W_t/P_t=MRS_t(1+\varphi)$, dropping the $\bar\mu^w$ factor. For the actual
(sticky-wage) economy the equilibrium condition (56) is what pins the steady state, so
\eqref{eq:ss-wage} --- markup included --- is the correct target when calibrating $\bar\xi$ and the
labor share. Keep this in mind together with \hyperref[rem:util]{Remark~\ref*{rem:util}}
(the $\bar R^k$/$\kappa_a$/$\bar\tau^k$ link).
\end{remark}
